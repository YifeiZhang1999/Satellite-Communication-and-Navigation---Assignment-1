...
%% Plot the ACF of the multiple correlator --sbs
    acf=trackResults(1).I_multi{6000};
    if acf(6)<0
        acf=-acf;
    end
    figure;
    plot(-0.5:0.1:0.5,acf,'b-');hold on;
    scatter(-0.5:0.1:0.5,acf,'black');
    xlabel('code delay');
    ylabel('ACF');
    title('ACF of Multi-correlator');

% %% Plot the velocity of SPV and EKF --sbs
%     figure;
%     subplot(navSolutions.VX_kf,'r');
